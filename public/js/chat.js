...
document.onkeydown = function keyDown(e) {
    if (e.keyCode === 13) {
        if ($('#chatText').val != "") {
            ...
            $('#chatHistory').append($('<div>').addClass("chatMsg").text($('#chatText').val()));
            $('#chatText').val("");
...
$("#audioPhone").click(function () {
    ...
    if (openAudioPhone % 2 == 0) {
        offOrOn = false;
        $("#audioPhone").removeClass("audioPhoneT").addClass("audioPhoneO");
        closeAudio();
    }
    openAudioPhone++;
});
...
function shortAudio() {
    ...
    document.onkeyup = function keyUp(e) {
        ...
        if (e.keyCode === 27) {
            if (door) {
                //语音聊天发送数据
                socket.emit('shortAudio', gRecorder.getBlob());
                console.log("mes" + gRecorder.getBlob().size);
                $('#chatHistory').append($('<audio>').attr({
                    'src': window.URL.createObjectURL(gRecorder.getBlob()),
                    'controls': "controls",
                }).addClass("chatAudio"));
                historyScroll.scrollTop = historyScroll.scrollHeight;
...
(function () {
    ...
    socket.on('chat message', function (mess) {
        $('#chatHistory').append($('<div>').addClass("chatMsgOther").text(mess));
        historyScroll.scrollTop = historyScroll.scrollHeight;
    });
    ...
    //短连接
    socket.on('shortAudio', function (audioData) {
        var dataView = new DataView(audioData);
        var blob = new Blob([dataView], {type: 'audio/wav'});
        // audio.src = (window.URL.createObjectURL(blob));
        console.log("rec" + blob.size);
        $('#chatHistory').append($('<audio>').attr({
            'src': window.URL.createObjectURL(blob),
            'controls': "controls",
            'autoplay': "autoplay"
